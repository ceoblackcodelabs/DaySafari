...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'DaySafaris.settings')
django.setup()

from Home.models import Trekking, ItineraryTreking
from decimal import Decimal

def seed_trekking_packages():
    print("Seeding trekking packages...")
    ...
    kilimanjaro = Trekking.objects.create(
    ...
    print(f"✓ Created {kilimanjaro.name} with {len(itinerary_kili)} days")

    # ==================== MOUNT KENYA - 5 DAYS SIRIMON ROUTE ====================
    mount_kenya = Trekking.objects.create(
    ...
    print(f"✓ Created {mount_kenya.name} with {len(itinerary_kenya)} days")

    # ==================== MOUNT LONGONOT - 1 DAY HIKE ====================
    longonot = Trekking.objects.create(
    ...
    print(f"✓ Created {longonot.name} with {len(itinerary_longonot)} days")

    # ==================== MOUNT SUSWA - 2 DAYS VOLCANIC ADVENTURE ====================
    suswa = Trekking.objects.create(
    ...
    print(f"✓ Created {suswa.name} with {len(itinerary_suswa)} days")

    print("\n" + "="*50)
    print("SEEDING COMPLETED SUCCESSFULLY!")
    print("="*50)
    print(f"Total Trekking Packages: {Trekking.objects.count()}")
    print(f"Total Itinerary Days: {ItineraryTreking.objects.count()}")
    print("\nPackages created:")
    for package in Trekking.objects.all():
        print(f"  • {package.name} ({package.days} days)")
        print(f"    Itinerary days: {package.itinerary_days.count()}")

if __name__ == "__main__":
    seed_trekking_packages()